					...
					uint8_t filter, uint8_t mode, uint8_t t_sb)
{
	...
	uint8_t state = I2C_ReadData(BME280_I2C_ADDR, ID_REG, &ID, 1);
	// printf("ReadData state = %d\r\n", state);
...
uint32_t BME280_CompensateH(int32_t adc_H)
{
	...
	v_x1_u32r = (((((adc_H << 14) - (((int32_t )CalibParam.dig_H4) << 20) - (((int32_t )CalibParam.dig_H5) * v_x1_u32r)) +
		((int32_t )16384)) >> 15) * (((((((v_x1_u32r * ((int32_t )CalibParam.dig_H6)) >> 10) * (((v_x1_u32r *
		((int32_t )CalibParam.dig_H3)) >> 11) + ((int32_t )32768))) >> 10) + ((int32_t )2097152)) *
